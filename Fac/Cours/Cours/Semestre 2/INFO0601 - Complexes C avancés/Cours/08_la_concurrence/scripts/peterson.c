/**
 * This program simulates the Peterson algorithm for the mutual exclusion.
 * - We use a shared memory segment for the variables (turn and states)
 ...
 * During execution (X is 0 or 1):
 * - X-W: process X is waiting
 * - X-E: process X enters in critical section
 * - X-E: process X leaves critical section
 * - Sleep is used to reduce CPU charge.
 ...
 * @param turn the turn (0 or 1)
 * @param states the states array
 */
void child(int num, int out, int *turn, int *states) {    
    // Main loop
    while(stop == 0) {
        ...
        *turn = num;
        printf("%d-W ", num);
        fflush(stdout);
        while((num == *turn) && (states[1 - num] == TRUE)) sleep(_random(1,2));
        printf("%d-E ", num);
        fflush(stdout);
...
int main() {
    ...
    if((shmid = shmget(CLE, sizeof(int) * 3, S_IRUSR | S_IWUSR | IPC_CREAT | IPC_EXCL)) == -1) {
    ...
    if((turn = shmat(shmid, NULL, 0)) == (void*)-1) {
