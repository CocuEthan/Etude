/**
 * This program simulates the strict alternation between two processes.
 * The processes alternatively enters in critical section.
...
void child(int num, int out, int *lock) {    
    ...
    while(stop == 0) {
        ...
        printf("%d-W ", num);
        fflush(stdout);
        while(*lock != num) sleep(_random(1, 2));
        printf("%d-E ", num);
        fflush(stdout);
