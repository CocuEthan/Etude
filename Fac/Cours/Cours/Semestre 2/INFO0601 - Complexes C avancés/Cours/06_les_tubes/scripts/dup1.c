/**
 * Use `dup` to create a redirection between 2 programs.
...
void child(int out) {
    ...
    if(close(STDOUT_FILENO) == -1) {
        perror("Child: error closing the standard output");
        exit(EXIT_FAILURE);
    }
    if(dup(out) == -1) {
    ...
    if(close(STDERR_FILENO) == -1) {
        perror("Child: error closing the standard error output");
        exit(EXIT_FAILURE);
    }
    if(dup(out) == -1) {
...
int main(int argc, char *argv[]) {
    ...
    if(close(STDIN_FILENO) == -1) {
        perror("Father: error closing standard input");
        exit(EXIT_FAILURE);
    }
    if(dup(_pipe[PIPE_READ]) == -1) {
