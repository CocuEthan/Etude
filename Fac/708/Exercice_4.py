...
# Importation des fonctions de base depuis le premier fichier
...
def calculer_nouveaux_centres_medians(liste_points, affectations, k, centres_precedents):
    ...
    for i in range(k):
        # Récupérer les points appartenant au cluster i
        points_cluster = []
        for j in range(len(liste_points)):
            if affectations[j] == i:
                points_cluster.append(liste_points[j])
        
        # Si le cluster est vide, le centre ne bouge pas
        if len(points_cluster) == 0:
            nouveaux_centres.append(centres_precedents[i])
        else:
            centre_median = []
            # Calcul de la médiane pour CHAQUE dimension (x, puis y, puis z...)
            for d in range(dim):
                valeurs_dim = []
...
def executer_kmedians(liste_points, centres_initiaux, max_iter=100, epsilon=1e-5):
    ...
    # 4. Calcul du coût final (avec L1)
