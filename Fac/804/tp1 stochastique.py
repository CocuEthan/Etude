...
# Boucle d'apprentissage Stochastique (SGD)
for epoch in range(n_epochs): 
    # 1. Mélanger les données au début de chaque époque
    indices = np.random.permutation(len(X))
    X_shuffled = X[indices]
    y_shuffled = y_true[indices]
    
    # 2. Mettre à jour les poids pour chaque point individuellement
    for i in range(len(X_shuffled)):
        ...
        z_i = forward(x_i, w, b)
        y_hat_i = perceptron_activation(z_i)
        
        # S'il y a une erreur de classification, on met à jour immédiatement
        if y_hat_i != y_i:
            delta_w = -y_i * x_i
            delta_b = -y_i
            w, b = update_weights(w, b, delta_w, delta_b, learning_rate)
            
    # 3. Calcul du coût global à la fin de l'époque pour le suivi
    y_hat_full = predict(X, w, b, perceptron_activation)
    loss = compute_error(y_true, y_hat_full, model_type='perceptron') 
...
for epoch in range(n_epochs): 
    ...
    # Sauvegarde des paramètres à la fin de l'époque pour l’animation 
    w_history.append(w.copy()) 
    b_history.append(b) 
    
    # Évaluation
    y_hat_full = predict(X, w, b, perceptron_activation)
    error = compute_error(y_true, y_hat_full, model_type='perceptron') 
